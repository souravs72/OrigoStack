...
const ResultsAnalysis: React.FC = () => {
  ...
  const generatePerformanceSummary = () => {
    ...
    const successRate = (selectedResult.successful_reqs / selectedResult.total_requests) * 100;
    const errorRate = (selectedResult.failed_requests / selectedResult.total_requests) * 100;
    ...
    return {
      ...
      efficiency_score: Math.min(100, Math.round((avgRps / selectedResult.config.max_rps) * 100)),
    };
  };
  ...
  const statusData = selectedResult ? {
    ...
    datasets: [
      {
        data: [selectedResult.successful_reqs, selectedResult.failed_requests],
        backgroundColor: [
          "rgba(76, 175, 80, 0.8)",
          "rgba(244, 67, 54, 0.8)",
        ],
                ...
                >
                  {filteredSimulations.map((sim) => (
                    <MenuItem key={sim.id} value={sim.id}>
                      {sim.name} - {new Date(sim.start_time).toLocaleDateString()} ({sim.status})
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Accordion defaultExpanded>
              ...
              <AccordionDetails>
                <Grid container spacing={3}>
                  {Object.entries(generatePerformanceSummary()).map(([key, value]) => (
              ...
              <CardContent>
                ...
                <Table size="small">
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell><strong>RPS Range:</strong></TableCell>
                      <TableCell>
                        {selectedResult.config.min_rps.toLocaleString()} - {selectedResult.config.max_rps.toLocaleString()}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Duration:</strong></TableCell>
                      <TableCell>
                        {Math.round(selectedResult.config.duration / 60000000000)} minutes
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Pattern:</strong></TableCell>
                      <TableCell>
                        <Chip label={selectedResult.config.pattern} size="small" />
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell><strong>Scale Mode:</strong></TableCell>
                      <TableCell>
                        <Chip label={selectedResult.config.scale_mode} size="small" />
                      </TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
                  ...
                  <Table size="small">
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell>{selectedResult.response_times.min.toFixed(2)}ms</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell><strong>Max:</strong></TableCell>
                        <TableCell>{selectedResult.response_times.max.toFixed(2)}ms</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell><strong>Mean:</strong></TableCell>
                        <TableCell>{selectedResult.response_times.mean.toFixed(2)}ms</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell><strong>P95:</strong></TableCell>
                        <TableCell>{selectedResult.response_times.p95.toFixed(2)}ms</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell><strong>P99:</strong></TableCell>
                        <TableCell>{selectedResult.response_times.p99.toFixed(2)}ms</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                ) : (
      ...
      {!selectedResult && !loading && (
        <Card>
          <CardContent sx={{ textAlign: "center", py: 6 }}>
            ...
            <Typography variant="body2" color="textSecondary">
              Choose from the dropdown above to view detailed performance analysis
            </Typography>
          </CardContent>
        </Card>
      )}
